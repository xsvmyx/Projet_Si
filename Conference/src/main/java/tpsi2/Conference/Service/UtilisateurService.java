...
public class UtilisateurService {
   ...
   //Methode pour voir si l'utilisateur a un role donné
    ...
    public boolean isAuteur(String soumissionNom){

        Optional<Utilisateur> user = getUserFromContext();
        Soumission soumission = soumissionRepository.findByTitre(soumissionNom);
        System.out.println(soumission.getTitre());
        Conference conference = soumission.getConference();


       return (userConferenceRoleRepository.existsByUserIdAndConferenceIdAndRole(user.get().getId(),conference.getId(), "AUTEUR") && soumission.getAuteurs().contains(user.get()));

    }

    public boolean isEvaluateur(Long evaluationId){

        Optional<Utilisateur> utilisateur = getUserFromContext();
       Optional< Evaluation > evaluation = evaluationRepository.findById(evaluationId);

        if(evaluation.isPresent()){
            if(evaluation.get().getEvaluateur().equals(utilisateur.get())){
                return true;
            }
            return false;
        }
